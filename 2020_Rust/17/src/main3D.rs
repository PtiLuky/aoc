...
/// FUNCTIONS /////////////////////////////////////////////////////////////////
fn xyz_to_idx(coords: (usize, usize, usize), dim: (usize, usize, usize)) -> usize {
    return coords.0 + coords.1 * dim.0 + coords.2 * dim.0 * dim.1;
}

#[allow(dead_code)]
fn print_cells(planete: &Planete, z: usize) {
    for y in 0..planete.dim.1 {
        for x in 0..planete.dim.0 {
            print!("{}", match planete.cells[xyz_to_idx((x,y,z), planete.dim)] { Active => '#', Inactive => '.'});
        }
        println!();
    }
    println!("{}, {}, {}", planete.dim.0, planete.dim.1, planete.dim.2);
}

fn count_neighbors(world: &Planete, coord: (usize, usize, usize)) -> usize {
    let mut count = 0;
    for dx in -1..=1 {
        ...
        for dy in -1..=1 {
            ...
            for dz in -1..=1 {
                ...
                // Actually count
                match world.cells[xyz_to_idx((x, y, z), world.dim)] {
                    Active => { count += 1; },
                    Inactive => {}
                }
            }
    ...
    for x in 0..planete.dim.0 {
        for y in 0..planete.dim.1 {
            for z in 0..planete.dim.2 {
                let cnt = count_neighbors(&current, (x, y, z));
                let idx = xyz_to_idx((x, y, z), planete.dim);
...
fn read_input(path: &str, nb_ite: usize) -> Planete { 
    ...
    let mut cells : Vec<Cell> = vec![Inactive; w * h * p];
    let z = nb_ite;
    let mut y = nb_ite;
    for l in f.lines() {
        let mut x = nb_ite;
        for c in l.chars() {
            cells[xyz_to_idx((x,y,z), (w,h,p))] = match c { '#' => Active, _ => Inactive }; 
            x += 1;
        }
        y += 1;
    }
    Planete{cells, dim: (w, h, p)}
}
